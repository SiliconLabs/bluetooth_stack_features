...
typedef struct rsp{
  ...
  uint32_t counter;
  sl_bt_evt_scanner_scan_report_t data;
}rsp_t;
...
int __match(const sl_bt_evt_scanner_scan_report_t *rsp, rsp_t *r);
void __copy(const sl_bt_evt_scanner_scan_report_t *rsp, rsp_t *r);
