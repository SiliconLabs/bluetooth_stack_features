...
SL_WEAK void app_init(void)
{
  ...
  sl_app_log("BLE SECURITY TEST APPLICATION : Starting application log... \r\n");
  sl_app_log("App init \r\n");
}
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
  ...
  case sl_bt_evt_system_boot_id:
    ...
    {
      sl_app_log("Clearing already stored bondings !\r\n");
  ...
  case sl_bt_evt_system_soft_timer_id:
    ...
    case INCREASE_SEC_TMR_HANDLE:
      sl_app_log("Increasing security after delay...\r\n");
  ...
  case sl_bt_evt_connection_opened_id:
    sl_app_log("Bluetooth Stack Event : CONNECTION OPENED\r\n");
    ...
    {
      sl_app_log("Increasing security\r\n");
    ...
    {
      sl_app_log("Already Bonded (ID: %d)\r\n", bleBondingHandle);
    }
    break;

  case sl_bt_evt_connection_parameters_id:
  {
    sl_app_log("Bluetooth Stack Event : CONNECTION Parameters ID\r\n");
    ...
    if (securityLevel > 2)
    {
      sl_app_log("[OK]      Bluetooth Stack Event : CONNECTION PARAMETERS : MTU = %d, SecLvl : %d, Timeout : %d\r\n", txSize, securityLevel, timeout);
    }
    else
    {
      sl_app_log("Bluetooth Stack Event : CONNECTION PARAMETERS : MTU = %d, SecLvl : %d, timeout : %d\r\n", txSize, securityLevel, timeout);
    }

    //If security is less than 2 increase so devices can bond
    if (securityLevel < 2)
    {
      sl_app_log("Bonding Handle is: 0x%04X\r\n", bleBondingHandle);
      if (bleBondingHandle == 0xFF)
      {
        sl_app_log("Increasing security.\r\n");
    ...
    {
      sl_app_log("Clear increase sec timer.\r\n");
  ...
  case sl_bt_evt_connection_closed_id:
    ...
    sl_app_log("Bluetooth Stack Event : CONNECTION CLOSED (reason: 0x%04X)\r\n", evt->data.evt_connection_closed.reason);
  ...
  case sl_bt_evt_sm_passkey_display_id:
    sl_app_log("Passkey %06ld\r\n", evt->data.evt_sm_passkey_display.passkey);
    sl_app_log("Bluetooth Stack Event : PASSKEY DISPLAY\r\n");
    break;

  case sl_bt_evt_sm_confirm_passkey_id:
    sl_app_log("Bluetooth Stack Event : PASSKEY CONFIRM\r\n");
    break;

  case sl_bt_evt_sm_passkey_request_id:
    sl_app_log("Bluetooth Stack Event : PASSKEY REQUEST\r\n");
    break;

  case sl_bt_evt_sm_confirm_bonding_id:
    sl_app_log("Bluetooth Stack Event : CONFIRM BONDING\r\n");
    uint8_t connectionhandle = evt->data.evt_connection_parameters.connection;
  ...
  case sl_bt_evt_sm_bonded_id:
    sl_app_log("Bluetooth Stack Event : BONDED\r\n");
    break;
  ...
  case sl_bt_evt_sm_bonding_failed_id:
  {
    ...
    sl_app_log("Bluetooth Stack Event : BONDING FAILED (connection: %d, reason: 0x%04X, bondingHandle: %d)\r\n",
               connectionHandle, reason, bleBondingHandle);
    if (reason == SL_STATUS_BT_SMP_PASSKEY_ENTRY_FAILED || reason == SL_STATUS_TIMEOUT)
    {
      sl_app_log("Increasing security... because reason is 0x%04x\r\n", reason);
      sc = sl_bt_sm_increase_security(connectionHandle);
      sl_app_assert(sc == SL_STATUS_OK,
                    "[E: 0x%04x] Failed to enhance the security\r\n",
                    (int)sc);
    ...
    if (reason == SL_STATUS_BT_SMP_PAIRING_NOT_SUPPORTED || reason == SL_STATUS_BT_CTRL_PIN_OR_KEY_MISSING)
    {
      if (bleBondingHandle != 0xFF)
      {
        sl_app_log("Broken bond, deleting ID:%d...\r\n", bleBondingHandle);
      ...
      {
        sl_app_log("Increasing security in one second...\r\n");
        uint16_t result;
        result = sl_bt_sm_increase_security(connectionHandle);
        sl_app_log("Result... = 0x%04X\r\n", result);
        ...
        {
          sl_app_log("Trying to increase security again");
          sc = sl_bt_sm_increase_security(connectionHandle);
          sl_app_assert(sc == SL_STATUS_OK,
                        "[E: 0x%04x] Failed to enhance the security\r\n",
                        (int)sc);
    ...
    {
      sl_app_log("Increasing security... because reason is 0x0308\r\n");
  ...
  case sl_bt_evt_gatt_server_execute_write_completed_id:
    sl_app_log("Bluetooth Stack Event : GATT SERVER WRITE COMPLETED\r\n");
    break;

  case sl_bt_evt_gatt_server_characteristic_status_id:
    sl_app_log("Bluetooth Stack Event : GATT SERVER CHARACTERISTIC STATUS\r\n");
    switch (evt->data.evt_gatt_server_characteristic_status.characteristic)
    {
    default:
      sl_app_log("GATT unknown...\r\n");
      break;
    }
    break;

  case sl_bt_evt_gatt_server_attribute_value_id:
    sl_app_log("Bluetooth Stack Event : GATT SERVER ATTRIBUTE VALUE\r\n");
    switch (evt->data.evt_gatt_server_attribute_value.attribute)
    {
    default:
      sl_app_log("unkown attribute???\r\n");
      break;
    }
    break;

  case sl_bt_evt_system_external_signal_id:
    sl_app_log("Bluetooth Stack Event : EXTERNAL SIGNAL\r\n");
    break;

  case sl_bt_evt_gatt_server_user_write_request_id:
    sl_app_log("Bluetooth Stack Event : GATT SERVER USER WRITE REQUEST\r\n");
    break;
  ...
  default:
    sl_app_log("Bluetooth Stack Event : UNKNOWN (0x%04lx)\r\n", SL_BT_MSG_ID(evt->header));
    break;
  }
}
